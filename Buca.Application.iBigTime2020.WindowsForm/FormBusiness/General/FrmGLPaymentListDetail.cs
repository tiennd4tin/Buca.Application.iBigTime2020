...
namespace Buca.Application.iBigTime2020.WindowsForm.FormBusiness.General
{
    ...
    public partial class FrmGLPaymentListDetail : FrmXtraBaseVoucherDetail, IGLPaymentListView, IAccountingObjectsView,
                                              ...
                                              IInvoiceTypiesView
    {
        ...
        private readonly GLPaymentListPresenter _glVoucherListPresenter;
        ...
        private IList<GLPaymentListDetailModel> listDetail;
        ...
        public FrmGLPaymentListDetail()
        {
            ...
            _glVoucherListPresenter = new GLPaymentListPresenter(this);
        }
        ...
        protected override void InitData()
        {
            ...
            if (ActionMode == ActionModeVoucherEnum.AddNew) KeyValue = ((GLPaymentListModel)MasterBindingSource.Current).RefId;
            else
            {
                if (MasterBindingSource.Current != null && KeyValue == null)
                    KeyValue = ((GLPaymentListModel)MasterBindingSource.Current).RefId;
            }

            _glVoucherListPresenter.Display(KeyValue);

            if (RefType == 0)
                RefType = (int)BuCA.Enum.RefType.GLPaymentList;
        }
        ...
        protected override bool ValidData()
        {
            ...
            var paymentVoucherDetails = GLPaymentListDetails;
        ...
        {
            new GLPaymentListPresenter(null).Delete(KeyValue);
        }
        ...
        private void btnSelectVoucher_Click(object sender, EventArgs e)
        {
            ...
            using (var frmParam = new FrmGLVoucherListParamater(GLPaymentListDetails))
            {
                frmParam.Text = @"Chọn chứng từ";
                if (frmParam.ShowDialog() == DialogResult.OK)
                {
                    //listDetail = frmParam.GlVoucherListDetailModels;
                    listDetail = GLPaymentListDetails;
                    foreach (GLPaymentListDetailModel item in frmParam.GlPaymentListDetailModels)
                        listDetail.Add(item);
        ...
        public IList<GLPaymentListDetailModel> GLPaymentListDetails
        {
            get
            {
                var glVoucherListDetails = new List<GLPaymentListDetailModel>();
                if (grdAccountingView.DataSource != null && grdAccountingView.RowCount > 0)
                {
                    ...
                    {
                        var rowVoucher = (GLPaymentListDetailModel)grdAccountingView.GetRow(i);
                ...
                if (glVoucherListDetails.Count > 0)
                {
                    if (grdAccountingDetailView.DataSource != null && grdAccountingDetailView.RowCount > 0)
                    {
                        ...
                        {
                            var rowVoucher = (GLPaymentListDetailModel)grdAccountingDetailView.GetRow(i);
                            if (rowVoucher != null)
                            {
            ...
            set
            {
                bindingSourceDetail.DataSource = value.OrderBy(c => c.SortOrder).ToList() ?? new List<GLPaymentListDetailModel>();
                grdAccountingView.PopulateColumns(value);
                grdAccountingDetailView.PopulateColumns(value);

                if (listDetail != null)
                {
                    bindingSourceDetail.DataSource = listDetail ?? new List<GLPaymentListDetailModel>();
        ...
        {
            var item = (GLPaymentListDetailModel)grdAccountingView.GetFocusedRow();
            if (item == null)
                return;
            ShowFormDetail(item);
        }
        ...
        void ShowFormDetail(GLPaymentListDetailModel item)
        {
