...
{
    public partial class FrmFADevaluations : FrmBaseVoucherList, IFADepreciationsView, IFixedAssetsView, IAccountsView, IDepartmentsView, IAccountingObjectsView,
        IBudgetSourcesView, IBudgetChaptersView, IBudgetKindItemsView
    {
        ...
        protected override void LoadDataIntoGrid()
        {
            ...
            _faDepreciationsPresenter.Display((int)RefType.FADevaluation);
        }
        ...
        protected override void LoadDataIntoGridDetail(string refId)
        {
            ...
            var columnsCollection = new List<XtraColumn>
            {
                ...
                new XtraColumn
                {
                    ColumnName = "Description", ColumnVisible = true, ColumnWith = 200, ColumnCaption = "Diễn giải", ColumnPosition = 5, AllowEdit = true
                },
                new XtraColumn
                {
        ...
        protected override bool ValidAddNew()
        {
            ...
            {
                // Check chứng từ khấu hao của kỳ đã chọn có tồn tại
                
                //var devaluationPeriodType = frmPeriodType.DevaluationPeriodType;
                var toDate = frmPeriodType.ToDate;
                var fromDate = frmPeriodType.FromDate;
                //GlobalVariable.DevaluationPeriod = devaluationPeriodType;
                var faDevaluations = _faDepreciationsPresenter.GetFADevaluations((int)RefType.FADevaluation, toDate, GlobalVariable.DevaluationPeriod);
                if (faDevaluations.Count > 0)
                {
                    XtraMessageBox.Show(string.Format("Chứng từ khấu hao {0} đã tồn tại!", frmPeriodType.PeriodTypeName), ResourceHelper.GetResourceValueByName("ResDetailContent"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return false;
                }
            }
            else
        ...
        protected override void ChangeFormDetail()
        {
            ...
            if (obj == null)
            {
                SetFormDetail((int)RefType.FADevaluation);
                return;
            }
