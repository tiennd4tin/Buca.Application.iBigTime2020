...
{
    public partial class FrmS105H1 :  FrmXtraBaseParameter, IBudgetSourcesView, IBudgetChaptersView, IBudgetKindItemsView, IBudgetExpensesView
    {
        #region Presenter
        /// <summary>
        ...
        public bool IsSummaryProject
        {
            get
            {
                if (cboBudgetExpense.EditValue.ToString() == "<<Tổng hợp>>")
                    return true;
                return false;
            }
        }
        ...
        public string ProjectCode
        {
            get
            {
                if ((cboBudgetExpense.EditValue.ToString() == "<<Tổng hợp>>") || cboBudgetExpense.EditValue.ToString() == "<<Tất cả>>")
                {
                    return null;
                }
                return cboBudgetExpense.EditValue.ToString();
            }
        }

        public string BudgetExpenseList
        {
            get
            {
                if (cboBudgetExpense.EditValue.ToString() == "<<Tổng hợp>>" || cboBudgetExpense.EditValue.ToString() == "<<Tất cả>>")
        ...
        /// <summary>
        /// Handles the Load event of the FrmS105H1 control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        private void FrmS105H1_Load(object sender, EventArgs e)
        {
            ...
            cboBudgetExpense.EditValue = @"<<Tổng hợp>>";
        }
        ...
        public IList<BudgetExpenseModel> BudgetExpenses {
            set
            {
                var result = new List<BudgetExpenseModel>
                {
                    new BudgetExpenseModel {BudgetExpenseCode = "<<Tổng hợp>>", BudgetExpenseName = "<<Tổng hợp>>"},
                    new BudgetExpenseModel {BudgetExpenseCode = "<<Tất cả>>", BudgetExpenseName = "<<Tất cả>>"}
                };
                result.AddRange(value);
                cboBudgetExpense.Properties.DataSource = result;
                cboBudgetExpense.Properties.ForceInitialize();
                cboBudgetExpense.Properties.PopulateColumns();
                var columnsCollection = new List<XtraColumn>();
                ...
                columnsCollection.Add(new XtraColumn { ColumnName = "BudgetExpenseType", ColumnVisible = false });
                foreach (var column in columnsCollection)
                {
                    if (column.ColumnVisible)
                    {
                        cboBudgetExpense.Properties.Columns[column.ColumnName].Caption = column.ColumnCaption;
                        cboBudgetExpense.Properties.SortColumnIndex = column.ColumnPosition;
                    }
                    else
                        cboBudgetExpense.Properties.Columns[column.ColumnName].Visible = false;
                }
                cboBudgetExpense.Properties.DisplayMember = "BudgetExpenseCode";
                cboBudgetExpense.Properties.ValueMember = "BudgetExpenseCode";
            }
        }
        ...
        private void rndOption_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (rndOption.SelectedIndex == 0)
            {
                ...
                cboBudgetExpense.EditValue = @"<<Tất cả>>";
            }
            else
            {
