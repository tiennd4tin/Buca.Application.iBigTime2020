/***********************************************************************
 * <copyright file="FrmChooseCaReceipt.cs" company="BUCA JSC">
...
namespace Buca.Application.iBigTime2020.WindowsForm.FormBusiness.Estimate
{
    ...
    public partial class FrmChooseCaReceipt : XtraForm, ICAReceiptsView, IBudgetSourcesView, IProjectsView, IFundStructuresView
    {
        ...
        #region Event
        /// <summary>
        /// Initializes a new instance of the <see cref="FrmChooseCaReceipt"/> class.
        /// </summary>
        public FrmChooseCaReceipt()
        {
            InitializeComponent();
        ...
        public void LoadgridView()
        {
            if (buCommitmentDetail != null)
            {
                ...
                grdCAReceiptView.PopulateColumns(buCommitmentDetail);
            }
            else
            {
                bindingSourceBUCommitment.DataSource = new List<BUCommitmentRequestDetailModel>();
                grdCAReceiptView.PopulateColumns(new List<BUCommitmentRequestDetailModel>());
            }
   ...
   new XtraColumn { ColumnName = "Description", ColumnVisible = false },
            ...
            grdCAReceiptView = InitGridLayout(columnsCollection, grdCAReceiptView);
            SetNumericFormatControl(grdCAReceiptView, true);
            grdCAReceiptView.OptionsView.ShowFooter = false;
        ...
        public string RefDetailIds
        {
            get
            {
                ...
                {
                    for (int i = 0; i < grdCAReceiptView.RowCount; i++)
                    {
                        if (Selection.IsRowSelected(i))
                        {
                            selectRefDetail += (selectRefDetail != "") ? "," + grdCAReceiptView.GetRowCellValue(i, "RefDetailId") : "" + grdCAReceiptView.GetRowCellValue(i, "RefDetailId");
                        }
                    }
                }
                if (selectRefDetail != "")
            ...
            get
            {
                ...
                if (Selection.SelectedCount > 0)
                {
                    for (int i = 0; i < grdCAReceiptView.RowCount; i++)
                    {
                        if (Selection.IsRowSelected(i))
                        {
                            selectRefDetail.Add(grdCAReceiptView.GetRowCellValue(i, "RefId").ToString());
                        }
                    }
                }
                return selectRefDetail;
        ...
        public IList<CAReceiptModel> CAReceipts
        {
            set
            {
                ...
                bindingSourceBUCommitment.DataSource = value.ToList();
                grdCAReceiptView.PopulateColumns(value);
